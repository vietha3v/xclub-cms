...
export default function TeamLeaderboard({ challengeId, leaderboard: propLeaderboard, isLoading: propIsLoading }: TeamLeaderboardProps) {
  // Fetch data từ API nếu không có props
  const [{ data: leaderboardData, loading: apiLoading, error }, refetch] = useAxios<ChallengeTeamLeaderboard[]>(
    `/api/challenges/${challengeId}/leaderboard/teams`
  );

  const leaderboard = propLeaderboard || leaderboardData || [];
  const isLoading = propIsLoading || apiLoading;
  ...
  if (error) {
    return (
      <ErrorState
        ...
        message="Có lỗi xảy ra khi tải bảng xếp hạng team"
        onRetry={() => refetch()}
      />
    );
  }
