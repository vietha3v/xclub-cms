...
export default function ClubSearch({ onSearch }: ClubSearchProps) {
  ...
  const searchSuggestions = async () => {
    try {
      setIsSearching(true);
      const response = await axiosInstance.get(`/api/clubs/search?q=${searchTerm}&limit=5`);
      const results = response.data;
      const clubNames = results.map(club => club.name);
      setSuggestions(clubNames);
  ...
  return (
    <div className="relative">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {['Chạy bộ', 'Marathon', 'Hà Nội', 'TP.HCM', '5km', '10km'].map((term) => (
          <button
            key={term}
            type="button"
            onClick={() => {
